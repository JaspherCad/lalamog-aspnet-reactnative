...
import { useProfiles } from '@/lib/fetchProfilesCopy';
import { supabase } from '@/lib/supabase';
import FontAwesome from '@expo/vector-icons/FontAwesome';
import { Redirect } from 'expo-router';
import React, { useCallback, useEffect, useRef, useState } from 'react';
import { Alert, Image, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
import { processSwipeAPI, ProfileData, SwipeData } from '@/api/axiosInstance';
...
export default function Home() {
    ...
    const { profiles, matchedProfiles, fetching, fetchMatchedProfiles } = useProfiles('swipe');
    ...
    useEffect(() => {
        if (profiles.length > 0) {
            ...
            console.log(`Profiles loaded in /(tabs)/index: ${profiles.length}`);
        }
        else {
            console.log(`No profiles found in /(tabs)/index`);
        }
    }, [profiles]);
    ...
    const handleSwipeLeft = useCallback(async (index: number) => {
        ...
        console.log(`🥊Swiped left on card ${profile.userId}`);
        await recordSwipe(profile.userId, 'left');
    }, [profiles]);

    const handleSwipeRight = useCallback(async (index: number) => {
        const profile = profiles[index];
        console.log(`🥊Swiped right on card ${profile.userId}`);
        await recordSwipe(profile.userId, 'right');
    }, [profiles]);
    ...
    const recordSwipe = async (swipeeId: string, direction: 'left' | 'right') => {
        ...
        try {
            ...
            if (result.isMatch) {
                Alert.alert(`You have a new match!`, `You matched with ${swipeeId}!`, [
                    { text: 'OK' },
                ]);
                // update future local states
                // TODO: refresh matched profiles list
            ...
            <View style={styles.container}>
                <SwipeCard
                    ...
                    customPreviewRender={(currentSelectedProfile) => {
                        ...
                        const fullImageUrl = currentSelectedProfile.profilePictureUrl
                            ? `${baseUrl}${currentSelectedProfile.profilePictureUrl}`
                            : undefined;
                    ...
                    customRender={(p) => { // I WILL PROVIDE THIS FUNCTION, self provide dapat
                        const levelLabel =
                            p.experienceLevel === 1
                                ? 'Beginner'
                                : p.experienceLevel === 2
                        ...
                        const API_BASE_URL = process.env.EXPO_PUBLIC_API_BASE_URL
                        const baseUrl = (API_BASE_URL ? API_BASE_URL.replace(/\/api$/, "") : "") || "http://localhost:5248"
                        const fullImageUrl = p.profilePictureUrl
                            ? `${baseUrl}${p.profilePictureUrl}`
                            : undefined;
                        ...
                        return (
                            <View style={styles.card}>
                                ...
                                <View style={styles.infoContainer}>
                                    ...
                                    {p.bio && <Text style={styles.bio}>{p.userId}</Text>}
